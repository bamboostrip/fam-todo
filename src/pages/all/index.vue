...
<template>
  <TodoListLayout listId="all" title="全部">
    ...
    <!-- 空状态 -->
    <EmptyState v-else :theme="currentTheme" message="此处显示所有列表中的任务（按列表分组）。" />
...
// 分组并排序：任务(tasks)最前，自定义列表按order，其余按名称
const groupedByList = computed(() => {
  const groups = new Map<string, Todo[]>()
  const todos = todosStore.allTodos as unknown as Todo[]
  todos.forEach((t) => {
