...
import { Plus, FolderPlus, CircleX, RefreshCw } from 'lucide-vue-next'
...
<template>
  <div class="p-4 border-t space-y-2">
    ...
    <div class="flex justify-between">
      ...
      <!-- 同步按钮 -->
      <Button
        @click="loadingStore.startSync"
        variant="ghost"
        size="sm"
        :disabled="loadingStore.isLoading"
        class="flex items-center gap-2 text-muted-foreground hover:text-foreground"
      >
        <RefreshCw class="w-4 h-4" :class="{ 'animate-spin': loadingStore.isLoading }" />
